...
class Quarto(config.db.Model):
    ...
    casa_id = config.db.Column(config.db.Integer, config.db.ForeignKey(Casa.id), 
                          nullable=False)
...
class Mobilia(config.db.Model):
    ...
    quarto_id = config.db.Column(config.db.Integer, config.db.ForeignKey(Quarto.id), 
...
class Proprietario(config.db.Model):
    ...
    casa = config.db.relationship("Casa", backref="proprietario")

    def __str__(self):
        s = f'Proprietario {self.nome}: {self.casa}'
        return s


if __name__ == "__main__": # teste das classes
    
    if config.os.path.exists(config.arquivobd): # se houver o arquivo...
        config.os.remove(config.arquivobd) # ...apagar!

    config.db.create_all() # criar tabelas
    ...
    config.db.session.add(q2)
    config.db.session.commit()

    print(q1, q2)

    p1 = (Proprietario(nome="Zanella", email="zanella@gmail.com", telefone="(22)2222-2222", casa=c1))
    ...
    for q in config.db.session.query(Quarto).filter(Quarto.casa_id == c1.id).all():
        print(q)
    ...
    print("*** TESTE exibindo proprietario")
    print(p1)